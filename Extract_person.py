...
class Detect_Person():
    ...
    def init_model(self):
        ...
        model = DetectMultiBackend(self.model_path, device=device, dnn=False, data=self.data_yaml, fp16=False)
        return model
...
def Extract_Person_frame(samples, frame_path, output_path, model_path, data_yaml):
    model = Detect_Person(model_path, data_yaml)
    for _, name in enumerate(samples):
        print("Processing " + name)
        label = name[-3:]
        setup_id = int(name[1:4])
        if int(setup_id) < 18: 
            sample_path = frame_path + '/NTU60/' + name + '/'
            save_path = output_path + '/NTU60/' + name + '/'
        else: 
            sample_path = frame_path + '/NTU120/' + name + '/'
            save_path = output_path + '/NTU120/' + name + '/'

        frame_files = os.listdir(sample_path)
        for idx in range(len(frame_files)):
            
            img_path = sample_path + str(idx) + '.jpg'
            img = cv2.imread(img_path)
            img1, img2, img3, num = model.extract_person(img, label)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            if num == 1:
                save_name = save_path + str(idx) + '.jpg'
                cv2.imwrite(save_name, img1)
            elif num == 2:
                save_name1 = save_path + str(idx) + '_p1.jpg'
                save_name2 = save_path + str(idx) + '_p2.jpg'
                save_name3 = save_path + str(idx) + '_mp.jpg'
...
if __name__ == "__main__":
    ...
    rgb_frame_path = args.frame_path
    output_path = args.output_path
    model_path = args.model_path
    data_yaml = args.data_yaml
    Extract_Person_frame(samples, rgb_frame_path, output_path, model_path, data_yaml)
    print("All done!")